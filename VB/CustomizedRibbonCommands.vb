        ...
        Public Property IsToolboxVisible() As Boolean
            ...
            Set(ByVal value As Boolean)
                If value <> IsToolboxVisible Then
                    If value Then
                        ToolboxControl.SetBinding(LayoutPanel.VisibilityProperty, New Binding() With { _
                            .Path = New PropertyPath("(0).PreviewIsOpen", ReportDesigner.DocumentSelectorProperty), .RelativeSource = New RelativeSource(RelativeSourceMode.Self), .Converter = New BooleanToVisibilityConverter() With {.Inverse = True} _
                        })
                    Else
        ...
        Public Property IsSidePanelVisible() As Boolean
            ...
            Set(ByVal value As Boolean)
                If value <> IsSidePanelVisible Then
                    If value Then
                        SidePanelControl.SetBinding(LayoutPanel.VisibilityProperty, New Binding() With { _
                            .Path = New PropertyPath("(0).PreviewIsOpen", ReportDesigner.DocumentSelectorProperty), .RelativeSource = New RelativeSource(RelativeSourceMode.Self), .Converter = New BooleanToVisibilityConverter() With {.Inverse = True} _
                        })
                    Else
